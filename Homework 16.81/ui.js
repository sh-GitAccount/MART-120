...
function DrawButton(x, y, w, h, label) {
  ...
  // EXPLICITLY RESET ALL COLOR STATES
  resetMatrix();
  noStroke();
  fill(100, 150, 255);
...
function DrawMenuScreen() {
    ...
    // Update pulse alpha
    menuPulseAlpha += menuPulseDir * 3; // adjust speed here
    if (menuPulseAlpha >= 255) {
        menuPulseAlpha = 255;
        menuPulseDir = -1;
    } else if (menuPulseAlpha <= 0) {
        menuPulseAlpha = 0;
        menuPulseDir = 1;
    }
    ...
    stroke(255, 200, 30);
    strokeWeight(6);
    fill(255, 0, 0, menuPulseAlpha); // pulsing alpha
    text("Shooty McBullet Game", width / 2, height / 3);
    pop();
    ...
    fill(255);
    noStroke();
    text("Click START to begin. Or go shop, or options. Do what you wanna do.", width / 2, height / 2 - 50);
    ...
    text("Use WASD to move, Click to Shoot, Shift to Boost. Q to Shield, E for Special Ability, Z to open Menu.", width / 2, height - 60);
...
function DrawShopScreen() {
  ...
  textSize(20);
  text("Total Gold: " + totalGold, width / 2, 75);
  fill(255, 200, 50);
  text("Gold Spent: " + goldSpent, width / 2, 100);
  pop();
...
function drawShopConfirmationTooltip(id, gridStartX, gridStartY, rows, cols, cellSize, gap, gridWidth) {
  ...
  const currentLevel = attachmentLevels_Current[id] || 0;
  const attCurrent = GetCurrentAttachmentData(id);  // Current level being upgraded FROM
  const nextLevelData = GetAttachmentLevelData(id, currentLevel + 1);  // Next level being upgraded TO
...
function drawStaticShopUpgradeTooltip(upgradeId, x, y) {
  ...
  if (!isUnlocked && upgCurrent.cost) {
    ...
    const nextLevelData = GetUpgradeLevelData(upgradeId, nextLevel);
    if (nextLevelData && nextLevelData.cost) {
      const canAfford = totalGold >= nextLevelData.cost;
      fill(canAfford ? 150 : 200, canAfford ? 255 : 100, canAfford ? 100 : 100);
      text("Upgrade Cost: " + nextLevelData.cost + "g", boxX + padding, boxY + padding + 28);
    }
  }
...
function drawStaticShopTooltip(attachmentId, gridCenterX, y) {
  ...
  if (!isOwned && attCurrent.cost) {
    ...
    const nextLevel = currentLevel + 1;
    const nextLevelData = GetAttachmentLevelData(attachmentId, nextLevel);
    if (nextLevelData && nextLevelData.cost) {
      const canAfford = totalGold >= nextLevelData.cost;
      fill(canAfford ? 150 : 200, canAfford ? 255 : 100, canAfford ? 100 : 100);
      text("Upgrade Cost: " + nextLevelData.cost + "g", boxX + padding, boxY + padding + 28);
...
function drawShopUpgradeConfirmationTooltip(id, gridStartX, gridStartY, rows, cols, cellSize, gap) {
  ...
  if (!isUnlocked) {
    ...
    const currentLevel = upgradeLevels_Current[id] || 0;
    const nextLevel = currentLevel + 1;
    const nextLevelData = GetUpgradeLevelData(id, nextLevel);
    if (nextLevelData && nextLevelData.cost) {
      text("Upgrade to Level " + (currentLevel + 1) + " for: " + nextLevelData.cost + "g", tooltipX + padding, tooltipY + padding + 50);
    } else {
      text("Already at MAX LEVEL", tooltipX + padding, tooltipY + padding + 50);
...
function DrawOptionsScreen() {
  ...
  // ===== FPS TOGGLE =====
  const fpstoggleY = 215;
  const fpstoggleSize = 40;
  const fpstoggleX = width / 2 - 100;

  strokeWeight(1);
  fill(200);
  textSize(24);
  textAlign(LEFT, CENTER);
  text("Display FPS:", width / 2 - 300, fpstoggleY);
  ...
  stroke(200);
  strokeWeight(2);
  rect(fpstoggleX, fpstoggleY - fpstoggleSize / 2, fpstoggleSize, fpstoggleSize, 5);
  ...
  textSize(20);
  textStyle(BOLD);
  text(showFPS ? "ON" : "OFF", fpstoggleX + fpstoggleSize / 2, fpstoggleY);

  // Check if toggle clicked
  if (mouseIsPressed && optionsMouseReleased &&
      mouseX > fpstoggleX && mouseX < fpstoggleX + fpstoggleSize &&
      mouseY > fpstoggleY - fpstoggleSize / 2 && mouseY < fpstoggleY + fpstoggleSize / 2) {
      
    optionsMouseReleased = false;

    showFPS = !showFPS; 
    playSound(showFPS ? 'confirm' : 'cancel');
    console.log("FPS display toggled to: " + showFPS);
  }
...
function applySFXVolume() {
  for (let soundName in sounds) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      } else if (typeof snd.volume === "function") {
...
function DrawVictoryScreen() {
  ...
  fill(255, 170, 0);
  textSize(24);
  text("Stage " + stage + " Complete!", width / 2, height / 3 + 80);
  ...
  if (stage < 4) {
    if (DrawButton(width / 2 - 180, buttonY, 160, 50, "NEXT STAGE")) {
      totalGold += Gold;
      Gold = 0;
      playSound('confirm');
...
function DrawNebulaLayer() {
  ...
  for (let x = startX; x < endX; x += cellSize) {
    for (let y = startY; y < endY; y += cellSize) {
      ...
      let saturation = map(noiseVal, 0, 1, stageTheme.nebulaSatMin, stageTheme.nebulaSatMax);
      let brightness = map(noiseVal, 0, 1, stageTheme.nebulaBrightMin, stageTheme.nebulaBrightMax);
...
function DrawStarLayer() {
  ...
  for (let x = startX; x < camX + width + starCell; x += starCell) {
    for (let y = startY; y < camY + height + starCell; y += starCell) {
      ...
      // small stars
      if (noiseVal > stageTheme.starDensitySmall) {
        
        fill(
          random(stageTheme.starHueMin, stageTheme.starHueMax),
          random(stageTheme.starSatMin, stageTheme.starSatMax),
          random(stageTheme.starBrightMin, stageTheme.starBrightMax),
          255
        );
      ...
      // big stars
      if (noiseVal > stageTheme.starDensityBig) {
...
function UpdateHud() {
  ...
  if (shield) {
    let shieldCdPercent = shield.onCooldown
      ? Math.floor((shield.timer / shield.cooldown) * 100)
      : 0;
    text("Shield", 860, 16);
    text("CD: " + shieldCdPercent + "%", 860, 36);
  }

  // Draw Ship Ability
  textSize(22);
  if (currentShipAbility) {
    const ship = shipAbilities[currentShipAbility];
    if (ship) {
      let shipCdPercent = ship.onCooldown
        ? Math.floor((ship.timer / ship.cooldown) * 100)
        : 0;
      text(ship.name || currentShipAbility, 1020, 16);
      text("CD: " + shipCdPercent + "%", 1020, 36);
    }
  }
...
// Used for selecting stage
function DrawStageSelectionScreen() {
  background(20, 20, 40);
  ...
  textSize(48);
  textStyle(BOLD);
  text("SELECT YOUR STAGE", width / 2, 100);

  textSize(18);
  fill(150);
  text("Complete stages to unlock the next one", width / 2, 160);
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    const isUnlocked = stagesUnlocked[i];
    const isCompleted = stagesCompleted[i];
    ...
    rect(x - boxSize / 2, stageY - boxSize / 2, boxSize, boxSize, 10);
    ...
    textSize(32);
    textAlign(CENTER, CENTER);
    text("Stage " + i, x, stageY - 20);

    if (isCompleted) {
      fill(255, 160, 0);
      textSize(14);
      text("COMPLETED", x, stageY + 20);
    } else if (!isUnlocked) {
      fill(150);
      textSize(14);
      text("LOCKED", x, stageY + 20);
    } else {
      fill(50, 180, 180);
      textSize(14);
      text("AVAILABLE", x, stageY + 20);
    }
  }
  ...
  fill(150);
  textAlign(CENTER);
  text("Click a stage to begin", width / 2, height - 20);
}
