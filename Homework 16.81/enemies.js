...
function KillEnemy(index) {
  ...
  if (enemy.type === "gigadia") {
    // Spawn dia in a circle pattern around spawn point
    for (let i = 0; i < 2; i++) {
  ...
  if (enemy.type === "superdia") {
    // Spawn 10 minidia in a circle pattern around spawn point
    for (let i = 0; i < 10; i++) {
      let angle = (i / 2) * TWO_PI;
      let offsetX = cos(angle) * 25;
      let offsetY = sin(angle) * 25;
...
function DrawBosses() {
  for (let boss of bosses) {
    ...
    if (boss.center.health > 0) {
      DrawBossPart(boss.worldX - cameraX, boss.worldY - cameraY, boss.center, 0, 150, 255);
    }
    // Draw left body (only if alive)
    if (boss.left.health > 0) {
      DrawBossPart(boss.worldX + boss.left.offsetX - cameraX, boss.worldY - cameraY, boss.left, 255, 100, 100);
    }
    // Draw right body (only if alive)
    if (boss.right.health > 0) {
      DrawBossPart(boss.worldX + boss.right.offsetX - cameraX, boss.worldY - cameraY, boss.right, 100, 255, 100);
    }
  }
}
...
function MoveEnemies() {
  ...
  let distToPlayerSq = distSquared(enemy.x, enemy.y, player_X, player_Y);
  if (distToPlayerSq > 4000000) { 
  enemies.splice(i, 1);
      continue;
    }
