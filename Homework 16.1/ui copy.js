...
function drawShopConfirmationBox(id) {
  ...
  // Draw semi-transparent background overlay (blocks clicks to grid)
  fill(0, 0, 0, 150);
  ...
  // Button positioning
  const buttonY = boxY + 180;
  const buttonWidth = 180;
  const buttonHeight = 50;
  const yesButtonX = boxX + 40;
  const noButtonX = boxX + boxW - 220;

  // YES BUTTON - Only if not maxed (or can be purchased/upgraded)
  if (!isMaxLevel) {
...
function DrawNebulaLayer() {
  ...
  for (let x = startX; x < endX; x += cellSize) {
    for (let y = startY; y < endY; y += cellSize) {
      ...
      // 3D noise: nebula "drifts" over time
      let noiseVal = noise(x * 0.006, y * 0.006, millis() * 0.00005);

      // nebula colors: purples & blues
      colorMode(HSB);
      let hueVal = map(noiseVal, 0, 1, 180, 260);  // blue → purple
