...
function UpdateHud() {
  ...
  if (abilities.length > 0) {
    const shieldAbility = abilityList.shield;
    let shieldCdPercent = shieldAbilityOnCooldown
      ? Math.floor((shieldAbilityCooldownTimer / shieldAbility.cooldown) * 100)
      : 0;
    text(shieldAbility.name, 860, 16);
  ...
  const shipAbility = abilityList[currentShipAbility];
  let shipCdPercent = shipAbilityOnCooldown
    ? Math.floor((shipAbilityCooldownTimer / shipAbility.cooldown) * 100)
    : 0;
    text(shipAbility.name, 1020, 16);
