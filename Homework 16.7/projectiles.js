...
function CheckShotCollisions() {
  for (let i = shot_X.length - 1; i >= 0; i--) {
    ...
    for (let b = bosses.length - 1; b >= 0; b--) {
      ...
      // Convert shot to SCREEN coordinates for collision with boss
      let shotScreenX = shot_X[i] - cameraX;
      let shotScreenY = shot_Y[i] - cameraY;
      let prevScreenX = prevX - cameraX;
      let prevScreenY = prevY - cameraY;
      
      // Check center body (only if alive)
      if (boss.center.health > 0) {
        const distToLineCenter = DistanceToLineSegment(boss.screenX, boss.screenY, prevScreenX, prevScreenY, shotScreenX, shotScreenY);
        ...
        if (distToLineCenter < collisionDistCenter) {
          ...
          if (boss.center.health <= 0 && boss.left.health <= 0 && boss.right.health <= 0) {
            ...
            if (bounce_Value > 0 && shot_BouncesRemaining[i] > 0) {
              BounceShot(i, boss.screenX + cameraX, boss.screenY + cameraY);
            } else {
              SpliceShot(i);
            }
          }
      ...
      if (boss.left.health > 0) {
        let leftScreenX = boss.screenX + boss.left.offsetX;
        const distToLineLeft = DistanceToLineSegment(leftScreenX, boss.screenY, prevScreenX, prevScreenY, shotScreenX, shotScreenY);
        ...
        if (distToLineLeft < collisionDistLeft) {
          ...
          if (boss.center.health <= 0 && boss.left.health <= 0 && boss.right.health <= 0) {
            ...
            if (bounce_Value > 0 && shot_BouncesRemaining[i] > 0) {
              BounceShot(i, leftScreenX + cameraX, boss.screenY + cameraY);
            } else {
              SpliceShot(i);
            }
          }
      ...
      if (boss.right.health > 0) {
        let rightScreenX = boss.screenX + boss.right.offsetX;
        const distToLineRight = DistanceToLineSegment(rightScreenX, boss.screenY, prevScreenX, prevScreenY, shotScreenX, shotScreenY);
        ...
        if (distToLineRight < collisionDistRight) {
          ...
          if (boss.center.health <= 0 && boss.left.health <= 0 && boss.right.health <= 0) {
            ...
            if (bounce_Value > 0 && shot_BouncesRemaining[i] > 0) {
              BounceShot(i, rightScreenX + cameraX, boss.screenY + cameraY);
            } else {
              SpliceShot(i);
            }
          }
...
function FireBossWeapon(boss, bodyPart, bossIndex) {
  ...
  let originX = boss.screenX + cameraX;
  let originY = boss.screenY + cameraY;
...
function FireCenterBeamVolley(boss, bossIndex, config) {
  console.log("Boss firing CENTER BEAM VOLLEY");
  playSound('charging');
  playSound('beamshot'); 
  ...
  for (let beamNum = 0; beamNum < config.beamCount; beamNum++) {
    setTimeout(() => {
      ...
      const originX = boss.screenX + cameraX;
      const originY = boss.screenY + cameraY;
