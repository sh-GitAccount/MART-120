...
function HandleAttachmentClick(mouseX, mouseY) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      //  // If locked — try to buy it    not sure want buy from pause
...
function EquipAttachment(attachmentId) {
  ...
  if (attachmentId === 12) {
    // shot_Penetration = 0;       Not sure if I like this? It's kind of insane to have both
...
function GetNextUpgradeCost(id) {
  ...
  const nextLevel = currentLevel + 1;
  const nextData = GetUpgradeLevelData(id, nextLevel);
  return nextData ? nextData.cost : null;
}
