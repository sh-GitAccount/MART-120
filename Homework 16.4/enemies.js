...
function KillBoss(bossIndex) {
  ...
  playSound("boss1centerexplode");  // Or make this configurable too
...
function UpdateWaves() {
  ...
  // Use time elapsed since game actually started
  const nowSeconds = (millis() - gameStartTime) / 1000;
  for (let wave of activeWaves) {
    ...
    // Calculate elapsed time from when wave started, accounting for pause offsets while in menu/shop etc
    const elapsed = (millis() - wave.startMillis - wave.pauseOffset) / 1000;
...
function HandleWavePause(isPaused) {
  if (!activeWaves) return;
  
  for (let wave of activeWaves) {
    if (!wave.started) continue;
    
    if (isPaused) {
      // Game is being paused - save current time
      wave.pauseStartTime = millis();
    } else {
      // Game is being unpaused - add the pause duration to offset
      if (wave.pauseStartTime) {
        wave.pauseOffset += (millis() - wave.pauseStartTime);
        wave.pauseStartTime = 0;
      }
    }
  }
}
...
function initWaves(waves) {
  ...
  for (let wave of activeWaves) {
    wave.started = false;
    wave.startMillis = 0;
    wave.pauseOffset = 0; // keeps track of time paused
