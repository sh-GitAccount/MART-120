...
var singularity_Damage_Bonus =0;
var singularityDamageDealt =0;
...
const abilityList = {
  bomb: {
    ...
    effect: () => {
      ...
      DamageBossesInRadius(bombStats.radius, bombStats.damage);
      damage_Dealt += damageCount * bombStats.damage;
  ...
  rapidfirebarrage: {  // Fires auto targeting barrage of projectiles at nearest enemy
    name: "Rapidfire Barrage",
    cooldown: 900,
    effect: () => {
      console.log("RAPIDFIRE BARRAGE ACTIVATED!");
...
function UseShieldAbility() {
  ...
  if (shieldAbilityOnCooldown) {
    console.log("Shield on cooldown!");
    return;
  }
  
  shieldAbility.effect();
  shieldAbilityOnCooldown = true;
  
  let cooldownReduced = shieldAbility.cooldown - (shieldAbility.cooldown * (cooldown_Reduction / 100));
  shieldAbilityCooldownTimer = cooldownReduced;
}

// Used to activate the inherent ship ability
function UseShipAbility() {
  if (!currentShipAbility) {
    console.log("No ship ability!");
    return;
  }
  
  const shipAbility = abilityList[currentShipAbility];
  
  if (shipAbilityOnCooldown) {
    console.log("Ability on cooldown!");
    return;
  }
  
  shipAbility.effect();
  shipAbilityOnCooldown = true;
  
  let cooldownReduced = shipAbility.cooldown - (shipAbility.cooldown * (cooldown_Reduction / 100));
  shipAbilityCooldownTimer = cooldownReduced;
}
...
function CheckBoltCollision() {
 ...
 // Check bosses 
  for (let b = 0; b < bosses.length; b++) {
    ...
    for (let bolt of activeBolts) {
      for (let j = 0; j < bolt.length - 1; j++) {
        ...
        // Check center
        const distToLineCenter = DistanceToLineSegment(boss.worldX, boss.worldY, seg1.x, seg1.y, seg2.x, seg2.y);
        if (distToLineCenter < 26) {
          boss.center.health -= boltDamage;
          boltDamageThisFrame += boltDamage;
          playSound('hit');
          
          if (boss.center.health <= 0 && boss.left.health <= 0 && boss.right.health <= 0) {
            enemiesToKill.push({ type: 'boss', index: b });
          }
          break;
        }
        
        // Check left
        const distToLineLeft = DistanceToLineSegment(boss.worldX + boss.left.offsetX, boss.worldY, seg1.x, seg1.y, seg2.x, seg2.y);
        if (distToLineLeft < 26) {
          boss.left.health -= boltDamage;
          boltDamageThisFrame += boltDamage;
          playSound('hit');
        ...
        // Check right
        const distToLineRight = DistanceToLineSegment(boss.worldX + boss.right.offsetX, boss.worldY, seg1.x, seg1.y, seg2.x, seg2.y);
        if (distToLineRight < 26) {
          boss.right.health -= boltDamage;
          boltDamageThisFrame += boltDamage;
          playSound('hit');
  ...
  damage_Dealt += boltDamageThisFrame;
...
function DealSingularityDamage() {
  const damageAmount = 60 + singularityCaughtEnemies.length * 15;
  ...
  for (let i = singularityCaughtEnemies.length - 1; i >= 0; i--) {
    ...
    singularityDamageDealt += damageAmount;
    playSound("hit");
  ...
  // Damage bosses caught in singularity
  for (let b = bosses.length - 1; b >= 0; b--) {
    ...
    if (distToCenter < singularityRadius * 1.4) {
    ...
    if (distToLeft < singularityRadius * 1.4) {
    ...
    if (distToRight < singularityRadius * 1.4) {
  ...
  damage_Dealt += singularityDamageDealt;
}
