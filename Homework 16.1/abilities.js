...
function UpdateBarrage() {
  if (barrageShotsRemaining > 0 && game_State && barrageActive && game_Screen === "playing") {
    console.log("Barrage updating!"); // <--- helps you see if it’s firing
    barrageTimer++; 
    ...
    if (barrageTimer >= 6) {
      ...
      if (target) {
        ...
        // If no enemies then fire toward mouse (convert to world coords)
...
function UpdateDecimator() {
  if (decimatorShotsRemaining > 0 && game_State && game_Screen === "playing") {
    if (!decimatorActive) {
      shot_Penetration += 2;
...
function UpdateSpiral() {
  if (spiralShotsRemaining > 0 && game_State && game_Screen === "playing") {
    spiralTimer++;
...
function UpdateLightningBolt() {
  if (!boltActive && game_Screen === "playing") return;
  boltTimer++;
...
function UpdateSingularity() {
  if (!singularityActive && game_State && game_Screen === "playing") return;
  singularityTimer++;

  if (singularityTimer >= singularityDuration && game_State && game_Screen === "playing") {
...
function ResetAllAbilities() {
    console.log("ResetAllAbilities called. barrageActive =", barrageActive);
    ...
    // Support unit states - these need to be ARRAYS
    bit_CooldownTimer = [];
    bit_Angle = [];
    blaster_BurstActive = [];
    blaster_BurstTimer = [];
